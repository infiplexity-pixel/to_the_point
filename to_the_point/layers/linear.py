...
class Linear(torch.nn.Module):
    ...
    def fit_batch(self, X_batch, Y_batch, P_batch=None, *args, **kwargs):
        if self.in_features is None:
            self.in_features = X_batch.shape[-1]

        if self.weight is None:
            self.weight = torch.nn.Parameter(
                torch.randn((self.in_features, self.out_features), device=self.device)
            )
            self.bias = torch.nn.Parameter(
                torch.randn((1, self.out_features), device=self.device)
            )
            self.cross_var = torch.zeros((self.in_features, self.in_features), device=self.device)
            self.co_cross_var = torch.zeros((self.in_features, self.out_features), device=self.device)
            self.sum_X = torch.zeros((1, self.in_features), device=self.device)
